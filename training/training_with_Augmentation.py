...
with strategy.scope():
  ...
  dice_loss = sm.losses.DiceLoss(class_weights=np.array([0.002, 1.5, 1.5, 1.95])) 
#   focal_loss = sm.losses.CategoricalFocalLoss(class_indexes = [1,2,3])
  jaccard_loss =  sm.losses.JaccardLoss(class_weights=[0.002, 1.5, 1.5, 1.95])
  total_loss = dice_loss  + 2* jaccard_loss
...
### model training
path_checkpoint = 'weights/withoutbce.hdf5'
modelckpt_callback = keras.callbacks.ModelCheckpoint(
    monitor="val_iou_score",
    filepath=path_checkpoint,mode = 'max',
    verbose=1,
    save_best_only=True,
)
...
it = datagen.flow(X_train, y_train_cat, batch_size=8)

history1=model1.fit(it,steps_per_epoch=244,
         validation_data= (X_test,y_test_cat),epochs = 30,callbacks =[modelckpt_callback])




model1.save('withoutbce.hdf5')
