...
with strategy.scope():
  ...
  dice_loss = sm.losses.DiceLoss(class_weights=np.array([0.002, 1.5, 1.5, 1.5])) 
#   focal_loss = sm.losses.CategoricalFocalLoss(class_indexes = [1,2,3])
  ...
  # define model
  model1 = sm.Linknet(BACKBONE1, classes=n_classes, activation=activation)
...
### model training
path_checkpoint = 'weights/linknet.hdf5'
